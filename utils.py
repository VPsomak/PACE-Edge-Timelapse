...
class Visualizer():
    ...
    def visualize_full(self,filename:str = None):
        """ Visualize the complete graph """
        plt.figure(figsize=self.figsize)
        pos = self.layout(self.graph,weight=lambda edge, data: (1.0 / data['time']) if data['time'] else 1.0)
        networkx.draw(
            self.graph, 
            pos, 
            with_labels=True, 
        ...
        if len(self.hosts) > 0 and len(self.hosts) < len(list(self.graph)):
            networkx.draw_networkx_nodes(self.graph, pos, nodelist=self.hosts, node_color='red', node_size=self.node_size)
        if len(self.active_nodes) > 0:
            networkx.draw_networkx_nodes(self.graph, pos, nodelist=self.active_nodes, node_color='green', node_size=self.node_size)
